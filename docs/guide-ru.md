...
> Статус: Desktop Connector `v0.6.0-beta.6`, Nexus-плагин `v0.6.0-beta.4`. Начинай с режима «Только чтение». Windows‑приложение пока не имеет коммерческой Authenticode‑подписи, поэтому SmartScreen может показать предупреждение.
...
На вкладке **Безопасность** сначала выбирается общее правило или конкретный разрешённый проект. Проект может наследовать общие настройки либо иметь собственные права на файлы и сеть:
...
Для сети доступны только **Запрещена** и **Запрашивать на этом ПК**. Во втором случае сетевой доступ подтверждается локально и выдаётся только на текущий turn с дополнительной проверкой последующих команд. Постоянного бесконтрольного сетевого доступа нет.

Даже полный доступ к файлам не разрешает произвольный доступ ко всему компьютеру: выход за выбранный проект запрещён, опасные действия автоматически не выполняются. Режим `dangerFullAccess` отсутствует намеренно.
...
Автоматического выполнения быть не должно. Сеть либо отключена, либо требует отдельного локального подтверждения; доступ к файлам ограничен выбранным проектом, а потенциально опасное действие требует решения пользователя. Если поведение отличается, останови turn голосовой командой или через Connector и отзови устройство в кабинете.
